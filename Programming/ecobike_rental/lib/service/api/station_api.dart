import 'dart:async';
import 'dart:convert';

import 'package:http/http.dart' as http;

import 'package:ecobike_rental/model/station.dart';
import '../util/converter.dart';
import 'response/station.dart';

class StationApi {
  Future<List<Station>> getListStation() async {
    List<Station> listStation;
    final response = await http
        .get('https://tkxdpm-server.herokuapp.com/api/get-list-stations');
    if (response.statusCode == 200) {
      listStation = (json.decode(response.body) as List)
          .map((i) => Converter.convertStationResponse(StationResponse.fromJson(i)))
          .toList();
    } else {
      throw Exception('Unable to fetch products from the REST API');
    }
    return listStation;
  }

  Future<Station> getStation(int stationId) async {
    Station station;
    final response = await http.get(
        'https://tkxdpm-server.herokuapp.com/api/get-station?id=${stationId}');
    if (response.statusCode == 200) {
      station = Converter.convertStationResponse(StationResponse.fromJson(json.decode(response.body)));
    } else {
      throw Exception('Unable to fetch products from the REST API');
    }
    return station;
  }
}
